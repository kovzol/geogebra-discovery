...
if __name__ == "__main__":
    ...
    CNI_THESIS_A_EXPRESSABLE = n()
    CNI_THESIS_A_NOW_EXPRESSABLE = n()
    CNI_SOLVING_FOR_A_REQUIRES_DIV_BY_B = n()
    CNI_THE_HYPOTHESES = n()
    CNI_PERP_PAR = n()
    CNI_EQ_COLL = n()
    CNI_THE_THESIS = n()
    CNI_ANGLE_AMBIGUITY = n()
    CNI_ELIMINATE_FREEVARS = n()
    CNI_NO_CORRESPONDENCE = n()
    CNI_THESIS_CANNOT_BE_EXPRESSED = n()
    CNI_THESIS_ZERO_TRUE = n()
    CNI_THESIS_SHOULD_BE_ZERO = n()
    CNI_THESIS_CAN_BE_EXPRESSED_POLY = n()
    CNI_HREAL_TREAL = n()
    CNI_ASSUME_DIV_ZERO = n()
    CNI_DIV_CANNOT_BE_ZERO = n()
    CNI_ASSUMING_ZERO_THESIS_CANNOT_BE_EXPRESSED = n()
    ...
    m(CNI_THESIS_A_EXPRESSABLE, 'ThesisACanBeExpressedAsRationalBecauseALinear')
    m(CNI_THESIS_A_NOW_EXPRESSABLE, 'ThesisACanBeExpressedNowAsRationalBecauseALinear')
    m(CNI_SOLVING_FOR_A_REQUIRES_DIV_BY_B, 'SolvingForARequiresDivByB')
    m(CNI_THE_HYPOTHESES, 'TheHypotheses')
    m(CNI_PERP_PAR, 'PerpendicularityParallelism')
    m(CNI_EQ_COLL, 'EqualityCollinearity')
    m(CNI_THE_THESIS, 'TheThesis')
    m(CNI_ANGLE_AMBIGUITY, 'AngleAmbiguity')
    m(CNI_ELIMINATE_FREEVARS, 'EliminateAllFreeVariables')
    m(CNI_NO_CORRESPONDENCE, 'NoCorrespondenceBetweenHypothesesThesis')
    m(CNI_THESIS_CANNOT_BE_EXPRESSED, 'ThesisCannotBeExpressed')
    m(CNI_THESIS_ZERO_TRUE, 'ThesisZeroStatementTrue')
    m(CNI_THESIS_SHOULD_BE_ZERO, 'ThesisShouldBeZero')
    m(CNI_THESIS_CAN_BE_EXPRESSED_POLY, 'ThesisCanBeExpressedPolynomial')
    m(CNI_HREAL_TREAL, 'HypothesesRealThesisReal')
    m(CNI_ASSUME_DIV_ZERO, 'AssumeDivisorZero')
    m(CNI_DIV_CANNOT_BE_ZERO, 'DivisorCannotBeZero')
    m(CNI_ASSUMING_ZERO_THESIS_CANNOT_BE_EXPRESSED, 'AssumingZeroThesisCannotBeExpressed')
    ...
    t(CNI_THESIS_A_NOW_EXPRESSABLE, HU, 'A következmény (%0) most megadható az előfeltételek racionális kifejezéseként, mivel %0 lináris a következő polinomegyenletben:', ZK);
    t(CNI_SOLVING_FOR_A_REQUIRES_DIV_BY_B, EN, 'Solving for %0 requires a division by %1.', ZK)
    t(CNI_SOLVING_FOR_A_REQUIRES_DIV_BY_B, HU, '%0 meghatározása egy %1-val/-vel osztást igényel.', ZK)
    t(CNI_THE_HYPOTHESES, EN, 'The hypotheses:', ZK)
    t(CNI_THE_HYPOTHESES, HU, 'Az előfeltételek:', ZK)
    t(CNI_PERP_PAR, EN, 'Perpendicularity means perpendicularity or parallelism simultaneously.', ZK)
    t(CNI_PERP_PAR, HU, 'A merőlegesség itt merőlegességet vagy párhuzamosságot egyszerre jelent.', ZK)
    t(CNI_EQ_COLL, EN, 'Equality of lengths means equality or collinearity simultaneously.', ZK)
    t(CNI_EQ_COLL, HU, 'Szakaszok egyenlősége itt egyenlőséget vagy kollinearitást egyszerre jelent.', ZK)
    t(CNI_THE_THESIS, EN, 'The thesis:', ZK)
    t(CNI_THE_THESIS, HU, 'A következmény:', ZK)
    t(CNI_ANGLE_AMBIGUITY, EN, 'Angle equality means equality or equality to another specific angle simultaneously.', ZK)
    t(CNI_ANGLE_AMBIGUITY, HU, 'Szögek egyenlősége itt egyenlőséget vagy egy másik konkrét szöggel való egyenlőséget egyszerre jelent.', ZK)
    t(CNI_ELIMINATE_FREEVARS, EN, 'We eliminate all variables that correspond to free points.', ZK)
    t(CNI_ELIMINATE_FREEVARS, HU, 'Minden szabad pontot reprezentáló változót eliminálunk.', ZK)
    t(CNI_NO_CORRESPONDENCE, EN, 'There is no correspondence between the hypotheses and the thesis.', ZK)
    t(CNI_NO_CORRESPONDENCE, HU, 'Nincs kapcsolat az előfeltételek és a következmény között.', ZK)
    t(CNI_THESIS_CANNOT_BE_EXPRESSED, EN, 'The thesis cannot be expressed with the hypotheses.', ZK)
    t(CNI_THESIS_CANNOT_BE_EXPRESSED, HU, 'A következmény nem fejezhető ki az előfeltételekkel.', ZK)
    t(CNI_THESIS_ZERO_TRUE, EN, 'Since the thesis is zero, the statement is true.', ZK)
    t(CNI_THESIS_ZERO_TRUE, HU, 'Mivel a következmény nulla, az állítás igaz.', ZK)
    t(CNI_THESIS_SHOULD_BE_ZERO, EN, 'Since the thesis is not zero, the statement cannot be proven.', ZK)
    t(CNI_THESIS_SHOULD_BE_ZERO, HU, 'Mivel a következmény nem nulla, az állítás nem bizonyítható.', ZK)
    t(CNI_THESIS_CAN_BE_EXPRESSED_POLY, EN, 'The thesis can be expressed as a polynomial expression of the hypotheses.', ZK)
    t(CNI_THESIS_CAN_BE_EXPRESSED_POLY, HU, 'A következmény kifejezhető az előfeltételek polinomjaként.', ZK)
    t(CNI_HREAL_TREAL, EN, 'Since all hypotheses are real expressions, the thesis must also be real.', ZK)
    t(CNI_HREAL_TREAL, HU, 'Mivel minden előfeltétel valós, a következménynek is valósnak kell lennie.', ZK)
    t(CNI_ASSUME_DIV_ZERO, EN, 'Let us assume that this divisor is 0 and restart the elimination.', ZK)
    t(CNI_ASSUME_DIV_ZERO, HU, 'Tegyük fel, hogy ez az osztó 0 és végezzünk újabb eliminációt.', ZK)
    t(CNI_DIV_CANNOT_BE_ZERO, EN, 'The elimination verifies that this divisor cannot be zero.', ZK)
    t(CNI_DIV_CANNOT_BE_ZERO, HU, 'Az elimináció igazolja, hogy az osztó nem lehet nulla.', ZK)
    t(CNI_ASSUMING_ZERO_THESIS_CANNOT_BE_EXPRESSED, EN, 'Assuming that this is zero, the thesis cannot be expressed with the hypotheses.', ZK)
    t(CNI_ASSUMING_ZERO_THESIS_CANNOT_BE_EXPRESSED, HU, 'Feltételezve, hogy ez nulla, a következmény nem fejezhető ki az előfeltételekkel.', ZK)
    t(CNI_NOW_THESIS_POLY, EN, 'Now the thesis can be expressed as a polynomial expression of the hypotheses.', ZK)
    t(CNI_NOW_THESIS_POLY, HU, 'Most a következmény kifejezhető az előfeltételek polinomjaként.', ZK)
    t(CNI_THESIS_STILL_DIV, EN, 'The thesis still contains a division, no conclusion can be found.', ZK)
    ...
    t(CNI_THE_HYPOTHESES, ES, 'Las hipótesis:', ZK)
    t(CNI_PERP_PAR, ES, 'La perpendicularidad significa perpendicularidad o paralelismo simultáneamente.', ZK)
    t(CNI_EQ_COLL, ES, 'La igualdad de longitudes significa igualdad o colinealidad simultáneamente.', ZK)
    t(CNI_THE_THESIS, ES, 'La tesis:', ZK)
    t(CNI_ANGLE_AMBIGUITY, ES, 'La igualdad de ángulos significa igualdad o igualdad con otro ángulo específico simultáneamente.', ZK)
    t(CNI_NO_CORRESPONDENCE, ES, 'No hay correspondencia entre las hipótesis y la tesis.', ZK)
    t(CNI_THESIS_CANNOT_BE_EXPRESSED, ES, 'La tesis no puede expresarse con las hipótesis.', ZK)
    t(CNI_THESIS_ZERO_TRUE, ES, 'Dado que la tesis es cero, la afirmación es verdadera.', ZK)
    t(CNI_THESIS_SHOULD_BE_ZERO, ES, 'Dado que la tesis no es cero, la afirmación no puede demostrarse.', ZK)
    t(CNI_THESIS_CAN_BE_EXPRESSED_POLY, ES, 'La tesis puede expresarse como una expresión polinómica de las hipótesis.', ZK)
    t(CNI_ASSUME_DIV_ZERO, ES, 'Supongamos que este divisor es 0 y reiniciemos la eliminación.', ZK)
    t(CNI_DIV_CANNOT_BE_ZERO, ES, 'La eliminación verifica que este divisor no puede ser cero.', ZK)
    t(CNI_ASSUMING_ZERO_THESIS_CANNOT_BE_EXPRESSED, ES, 'Suponiendo que esto es cero, la tesis no puede expresarse con las hipótesis.', ZK)
    t(CNI_NOW_THESIS_POLY, ES, 'Ahora la tesis puede expresarse como una expresión polinómica de las hipótesis.', ZK)
    t(CNI_THESIS_STILL_DIV, ES, 'La tesis todavía contiene una división, no se puede llegar a ninguna conclusión.', ZK)
    t(CNI_THESIS_CANNOT_BE_EXPRESSED_DIV, ES, 'La tesis no puede expresarse como una división.', ZK)
    ...
    t(SHOWPROOF, ZH, '显示证明', XCP)
    t(SHOWPROOF_SYNTAX, ZH, '[ <布尔表达式> ]', XCP)
    t(PROVE_THAT_A, ZH, '证明 %0。', XCP)
    t(LET_A_BE_ARBITRARY_POINTS, ZH, '设 %0 为任意点。', XCP)
    t(LET_A_BE_THE_B, ZH, '设 %0 为 %1。', XCP)
    t(LET_A_BE_A_B, ZH, '设 %0 为一个 %1。', XCP)
    t(LET_A_BE_THE_REGULAR_POLYGON_BCD, ZH, '设 %0 为线段 %1、%2 上的正 %3 边形。', XCP)
    t(LET_A_BE_THE_REGULAR_BGON_VERTICES_C, ZH, '设 %0 为顶点为 %2 的正 %1 边形。', XCP)
    t(DENOTE_THE_EXPRESSION_A_BY_B, ZH, '将表达式 %0 记为 %1。', XCP)
    t(PROOF_UNKNOWN, ZH, '该命题无法被证明或否定。', XCP)
    t(STATEMENT_ALWAYS_TRUE, ZH, '该命题始终为真。', XCP)
    t(TRUE_ON_PARTS, ZH, '该命题部分为真，部分为假。', XCP)
    t(TRUE_UNDER_NONDEGENERACY_CONDITIONS, ZH, '该命题在某些非退化条件下为真。', XCP)
    t(STATEMENT_FALSE, ZH, '该命题为假。', XCP)
    t(STATEMENT_TRIVIAL, ZH, '该命题显然成立。', XCP)
    t(PROVE_BY_CONTRADICTION, ZH, '采用反证法。', XCP)
    t(NO_FULL_PROOF, ZH, '目前软件只能证明部分步骤，无法提供完整证明。', XCP)
    t(NO_FULL_PRESENTATION, ZH, '内部所有步骤都已验证，但完整的展示尚未实现。', XCP)
    t(TRY_NEWER_VERSION, ZH, '如果可能，请尝试使用 GeoGebra Discovery 的新版本。', XCP)
    t(UNSUPPORTED_AXES_FIXED_SLOPE_LINES, ZH, '不支持包含坐标轴或用户固定斜率直线的命题。', XCP)
    t(CONSIDERING_DEFINITION_A, ZH, '考虑定义 %0：', XCP)
    t(LET_FREE_POINT_A_DENOTED_BY_B, ZH, '将自由点 %0 记为 %1。', XCP)
    t(ONLY_FIRST_FIXED_BECAUSE_A_ON_B, ZH, '只能固定第一个自由点，因为 %0 在直线 %1 上。', XCP)
    t(LET_DEPENDENT_POINT_A_DENOTED_BY_B, ZH, '将依赖点 %0 记为 %1。', XCP)
    t(OBJECT_A_INTRODUCES, ZH, '对象 %0 引入了以下额外变量：', XCP)
    t(COMMAND_A_NOT_FULLY_IMPLEMENTED, ZH, '命令 %0 在证明助手尚未完全实现。', XCP)
    t(COMMAND_A_NOT_IMPLEMENTED, ZH, '命令 %0 在证明助手未实现。', XCP)
    t(THESIS_EQS_NON_DENIED, ZH, '命题的等式（不否定的那些）：', XCP)
    t(THESIS_EQ_DENIED, ZH, '命题的否定，导致矛盾：', XCP)
    t(DUMMY_VAR_NEG, ZH, '用于表示否定的虚拟变量', XCP)
    t(STATEMENT_A_NOT_FULLY_IMPLEMENTED, ZH, '命题 %0 在证明助手尚未完全实现。', XCP)
    t(STATEMENT_A_NOT_IMPLEMENTED, ZH, '命题 %0 在证明助手未实现。', XCP)
    t(WLOG_COORDINATES, ZH, '不失一般性，可以固定某些坐标：', XCP)
    t(CANNOT_DECIDE_ALGEBRAIC_DIFFICULTIES, ZH, '抱歉，由于代数计算困难，程序无法得出结论。', XCP)
    t(WEAKENING_TRUE, ZH, '该命题的弱化版本为真。', XCP)
    t(STATEMENT_TRUE_NDG, ZH, '该命题可能在某些非退化条件下为真：', XCP)
    t(AFTER_SUBS, ZH, '代入之后：', XCP)
    t(STATEMENT_TRUE_NDG_UNREADABLE, ZH, '非退化条件本身无法用简单几何术语表达：', XCP)
    t(ALL_HYPOS_NEG_THESIS, ZH, '所有假设以及命题否定，代入之后：', XCP)
    t(NOW_CONSIDER, ZH, '现在考虑以下等式：', XCP)
    t(CONTRADICTION_THIS_PROVES, ZH, '矛盾！这证明了原命题。', XCP)
    t(DIFFICULTY_A, ZH, '该命题对应一个次数为 %0 的多项式。', XCP)
    t(PROOF_PREPARATION_SEEMS_DIFFICULT, ZH, '抱歉，证明的准备工作计算起来似乎太困难了。', XCP)
    t(FORCING_NON_COLLINEARITY, ZH, '强制某些三点组不共线。', XCP)
    t(THESIS_A_IN_ALGEBRAIC_FORM, ZH, '命题：%0，代数形式：', XCP)
    t(ONLY_FIRST_FIXED_EXPRESSION, ZH, '只能固定第一个自由点，因为命题是一个表达式。', XCP)
    t(A_VALUE_OF_CENTER_OF_B, ZH, '%0 为 %1 的中心的值', XCP)
    t(A_VALUE_OF_A_POINT_OF_B, ZH, '%0 为 %1 上一点的值', XCP)
    t(A_VALUE_OF_AN_IMPLICITLY_INTRODUCED_SECOND_POINT_FOR_ORTHOGONAL_LINE_AT_B_TO_C, ZH, '%0 为在 %1 处垂直于 %2 的直线上隐式引入的第二个点的值', XCP)
    t(A_VALUE_OF_MIDPOINT_OF_B, ZH, '%0 为 %1 中点的值', XCP)
    t(A_VALUE_OF_ROTATION_OF_POINT_B_AROUND_THE_MIDPOINT_OF_C_BY_D_DEGREES, ZH, '%0 为点 %1 绕 %2 中点旋转 %3 度后的值', XCP)
    t(VARIABLE_A_IS_ALREADY_DEFINED_PLEASE_REMOVE_IT_FIRST, ZH, '变量 %0 已定义。请先移除它。', XCP)
    t(PLEASE_OPEN_THE_CAS_VIEW_FIRST, ZH, '请先打开计算机代数系统视图。', XCP)
    t(DISCOVER, ZH, '发现', XCP)
    t(DISCOVER_TOOL, ZH, '发现工具', XCP)
    t(DISCOVER_TOOL_HELP, ZH, '选择一个点', XCP)
    t(DISCOVERED_THEOREMS_ON_POINT, ZH, '在点 %0 上发现的定理', XCP)
    t(DISCOVER_SYNTAX, ZH, '[ <点> ]', XCP)
    t(NO_THEOREMS_FOUND, ZH, '未发现定理。', XCP)
    t(REDRAW_DIFFERENTLY, ZH, '尝试以不同方式重新绘制图形。', XCP)
    t(UNSUPPORTED_STEPS, ZH, '图形包含不支持的步骤。', XCP)
    t(IDENTICAL_POINTS_A, ZH, '相同的点：%0', XCP)
    t(COLLINEAR_POINTS_A, ZH, '共线的点：%0', XCP)
    t(CONCYCLIC_POINTS_A, ZH, '共圆的点：%0', XCP)
    t(CONGRUENT_SEGMENTS_A, ZH, '全等的线段：%0', XCP)
    t(SETS_OF_PARALLEL_AND_PERPENDICULAR_LINES_A, ZH, '平行和垂直的直线组：%0', XCP)
    t(IN_PROGRESS, ZH, '正在进行中', XCP)
    t(CANNOT_DECIDE_EQUALITY_OF_POINTS_A_B, ZH, '无法判定点 %0 和 %1 是否相等。', XCP)
    t(COMPARE, ZH, '比较', XCP)
    t(COMPARE_SYNTAX, ZH, '[ <表达式>, <表达式> ]', XCP)
    t(COMPARE_A_AND_B, ZH, '比较 %0 和 %1。', XCP)
    t(INCIRCLECENTER, ZH, '内切圆圆心', XCP)
    t(INCIRCLECENTER_SYNTAX, ZH, '[ <点>, <点>, <点> ]', XCP)
    t(INCIRCLECENTER_TOOL, ZH, '内切圆圆心工具', XCP)
    t(INCIRCLECENTER_TOOL_HELP, ZH, '选择三个点', XCP)
    t(INCIRCLE_TOOL, ZH, '内切圆工具', XCP)
    t(INCIRCLE_TOOL_HELP, ZH, '选择三个点', XCP)
    t(LOCUSEQUATION_TOOL, ZH, '轨迹方程工具', XCP)
    t(LOCUSEQUATION_TOOL_HELP, ZH, '选择描述轨迹的点，然后选择对象上的点', XCP)
    t(ENVELOPE_TOOL, ZH, '包络工具', XCP)
    t(ENVELOPE_TOOL_HELP, ZH, '选择路径，然后选择对象上的点', XCP)
    t(STATEMENT_REQUIRES_CONDITIONS, ZH, '该命题需要一些条件：', XCP)
    t(A_VALUE_OF_MIDPOINT_OF_RHOMBUS_ANGULAR_BISECTOR_B, ZH, '%0 为（角平分线 %1 将穿过的）菱形中点的值', XCP)
    t(A_VALUE_OF_HELPER_POINT_MIRROR_B_C_ABOUT_D_E, ZH, '%0 为 (%1,%2) 关于 (%3,%4) 的对称辅助点的值', XCP)
    t(PROVER_TIMEOUT, ZH, '证明子系统已超时，抱歉。', XCP)
    t(GEOGEBRA_CANNOT_CHECK, ZH, 'GeoGebra 无法检查这是否等价于 1 = 0，但可以在其他计算机代数系统中计算。', XCP)
    t(REALQUANTIFIERELIMINATION, ZH, '实量词消去', XCP)
    t(REALQUANTIFIERELIMINATION_SYNTAX, ZH, '[ <表达式> ]', XCP)
    t(STEPWISEDISCOVERY, ZH, '逐步发现', XCP)
    t(STEPWISEDISCOVERY_SYNTAX, ZH, "[ ]\n[ <布尔值> ]", XCP)
    t(STEPWISEDISCOVERY_INFORMATION, ZH, '信息', XCP)
    t(STEPWISEDISCOVERY_ENABLED, ZH, '逐步发现功能已启用。', XCP)
    t(STEPWISEDISCOVERY_DISABLED, ZH, '逐步发现功能未启用。', XCP)
    t(STEPWISEDISCOVERY_STYLEBAR, ZH, '切换开关：逐步发现', XCP)
    t(EXPORT_AS_LATEX, ZH, '导出为 LaTeX', XCP)
    t(CAS_MAPLE_TO_CLIPBOARD, ZH, '将计算机代数系统视图复制到 Maple 剪贴板', XCP)
    t(CAS_GIAC_TO_CLIPBOARD, ZH, '将计算机代数系统视图复制到 Giac 剪贴板', XCP)
    t(CLEAR_CAS_VIEW, ZH, '清空计算机代数系统视图', XCP)
    t(EXPORT_CAS_MAPLE, ZH, '将计算机代数系统视图导出到 Maple', XCP)
    t(EXPORT_CAS_HTML, ZH, '将计算机代数系统视图导出为 HTML', XCP)
    t(EXPORT_CAS_LATEX, ZH, '将计算机代数系统视图导出到 LaTeX', XCP)
    t(EXPORT_CAS_MATHEMATICA, ZH, '将计算机代数系统视图导出到 Mathematica', XCP)
    t(EXPORT_CAS_GIAC, ZH, '将计算机代数系统视图导出到 Giac', XCP)
